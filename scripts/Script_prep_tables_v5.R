...
painel_lvs_secao <- read_csv("../LV_dados/painel_locvot_secao.csv",
...
painel_lvs_secao_all <- painel_lvs_secao %>% 
  select(ANO, CD_LOCALIDADE_TSE, NR_ZONA, NR_LOCVOT, NR_SECAO, QTD_ELEITORES, lat, lon) %>%
...
votes_secao <- files %>% map(read_delim, delim=";", 
  ...
  filter(SIGLA_UF=="SP"|CODIGO_MUNICIPIO==60011)
