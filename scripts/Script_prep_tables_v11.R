...
painel_lvs_secao_all <- painel_lvs_secao %>% 
  select(ANO, CD_LOCALIDADE_TSE, NR_ZONA, NM_LOCVOT, NR_LOCVOT, NR_SECAO, lat, lon) %>%
...
painel_lvs_secao_votos_inner_1 <- painel_lvs_secao_votos_inner_1 %>% 
  group_by(ANO_ELEICAO, SIGLA_UF, SIGLA_UE, CODIGO_MUNICIPIO, COD_MUN_IBGE, NUM_ZONA, NM_LOCVOT, NR_LOCVOT, CODIGO_CARGO, NUM_TURNO, NUM_VOTAVEL, lat, lon) %>%
...
painel_lvs_secao_votos_inner_lv <- painel_lvs_secao_votos_inner_lv %>%
  group_by(ANO_ELEICAO, SIGLA_UF, SIGLA_UE, CODIGO_MUNICIPIO, COD_MUN_IBGE, NUM_ZONA, NM_LOCVOT, NR_LOCVOT, CODIGO_CARGO, NUM_TURNO) %>%
...
painel_lvs_secao_votos_inner_mun_ano <- painel_lvs_secao_votos_inner_lv %>%
  nest(Zonas=c(NUM_ZONA, NM_LOCVOT, NR_LOCVOT,lat,lon,CODIGO_CARGO, NUM_TURNO, NUM_VOTAVEL, QTDE_VOTOS))

painel_lvs_secao_votos_inner_mun_ano_cargo_turno <- painel_lvs_secao_votos_inner_lv %>%
  select(-Largest_Party_LV) %>%
  nest(Zonas_LVs=c(NUM_ZONA, NM_LOCVOT, NR_LOCVOT,lat,lon, NUM_VOTAVEL, QTDE_VOTOS, Pct_Votos_LV))
